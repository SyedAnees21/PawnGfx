...
//         let v0_clip =
// transform_to_clip_space(&CUBE_VERTS[start],
// &mvp);         let v1_clip =
// transform_to_clip_space(&CUBE_VERTS[end],
// &mvp);
...
//         let (x0, y0, z0) =
// clip_to_screen(&v0_ndc, width, height);
//         let (x1, y1, z1) =
// clip_to_screen(&v1_ndc, width, height);
...
//     let w = global_uniforms.screen.width as
// i32;     let h = global_uniforms.screen.height
// as i32;

//     for (_idx, (v, n, uv)) in
// triangles.enumerate() {         let [v0, v1,
// v2] = v;

//         let v0_clip =
// transform_to_clip_space(v0, global_uniforms.
// affine.mvp);         let v1_clip =
// transform_to_clip_space(v1,
// global_uniforms.affine.mvp);         let
// v2_clip = transform_to_clip_space(v2,
// global_uniforms.affine.mvp);

//         if v0_clip.w <= 0.0 || v1_clip.w <= 0.0
// || v2_clip.w <= 0.0 {             continue;
//         }

//         let [n1, _, _] = n;

//         let face_normal =
// (global_uniforms.affine.normal *
// Vector4::from((n1, 0.0))).xyz();
...
// v1, v2);                 let inv_area = 1.0 /
// area;

//                 let w0 = edge_function(v1, v2,
// p) * inv_area;                 let w1 =
// edge_function(v2, v0, p) * inv_area;
//                 let w2 = edge_function(v0, v1,
// p) * inv_area;
...
//                 let inv_depth_cords = (inv_w0,
// inv_w1, inv_w2);

//                 let z =
// persp_correct_interpolate(bary_cords,
// inv_depth_cords, (z0, z1, z2));

//                 let depth_index = (y * w + x)
// as usize;                 let pixel_index =
// (depth_index * 4) as usize;

//                 if z <
// depth_buffer[depth_index] {
// let [uv0, uv1, uv2] = uv;
// let (uv0, uv1, uv2) = (uv0 * inv_w0, uv1 *
// inv_w1, uv2 * inv_w2);

//                     let u =
// persp_correct_interpolate(
// bary_cords,
// inv_depth_cords,
// (uv0.x, uv1.x, uv2.x),                     );
//                     let v =
...
//                     let s_color =
// texture.bi_sample(u, v);

//                     let intensity =
// face_normal.normalize().dot(&light).max(0.0);
