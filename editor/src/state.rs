...
use pscene::{assets, texture::{Albedo, NormalMap, Wrap}};
...
impl AssetRegistry {
    ...
    pub fn record_name(&self, id: AssetId) -> Option<&str> {
        self.records.iter().find(|r| r.id == id).map(|r| r.name.as_str())
    }
...
impl EditorState {
    ...
    pub fn assign_mesh_to_selected(&mut self, asset_id: AssetId) {
        let name = self.assets.record_name(asset_id).unwrap_or("Mesh").to_string();
    ...
    pub fn assign_albedo_to_selected(&mut self, asset_id: AssetId) {
        let name = self.assets.record_name(asset_id).unwrap_or("Albedo").to_string();
        if let Some(obj) = self.selected_object_mut() {
